...
\begin{lemma}
	\label{lemma:independence_number_gnp}
...
\subsubsection{Krivilevich lowerbound}
...
of a graph that satisfies [\ref{lemma:few_indep_r33k}].
...
\subsection{Ramsey of bounded degree graphs}

The motivation of this section will be the following theorem.
\begin{theorem}
	\label{trm:ramsey_bipartite_bounded_degree}
	Let $H$ be a bipartite graph with $k$ vertices and $\Delta(H) \leq d$. Then
	$r(H) \leq 2^{d + 6} k$.
\end{theorem}

\subsubsection{Old Friends}
To uncover it, we will need to remember some old forgotten friends.
The LLL and the DRC.

\begin{theorem}[Lovász Local Lemma]
	\label{trm:lll}
	Let $\mathcal{E} = \{B_1, B_2, \dots, B_m\}$ be bad events in some
	probability space and suppose that for each event $B_i$,
	there exists a set $N(B_i) \subset \mathcal{E}$ such that $B_i$ is
	mutually independent of all events in $\mathcal{E} \setminus N(B_i)$.
	We say that $j \sim i$ if $B_j \in N(B_i)$. If there exists $x_1,
	\dots x_m \in (0,1)$ satisfying
	\[
		\Pr(B_i) \leq x_i \prod_{j \sim i}(1-x_j)
	\]
	then
	\[
		\Pr\bigg(\bigcap_{i = 1}^m B_i^c\bigg) > \prod_{i=1}^m (1 - x_i).
	\]
\end{theorem}

\begin{remark}
	Usually people talk about the dependence graph, in this framework
	the neighbors of
	an event $B_i$ are precisely $N(B_i)$. Note that the choice of
	$N(B_i)$ (or of the dependece graph) is not necessairly unique.
\end{remark}

\begin{proof}[Proof of LLL \ref{trm:lll}]
	The proof is just a very smart induction (similar to Harris or
	Janson - which I should also add in the appendix).
	We lemma that given any $S \subset [m]$,
	\[
		\Pr\bigg(B_i \mid \bigcap_{j \in S} B_j^c \bigg) \leq x_i.
	\]
	Note that this implies the theorem as we can always write
	\[
		\Pr\bigg(\bigcap_{i = 1}^m B_i^c\bigg) = \prod_{i = 1}^m
		\Pr\bigg(B_i^c \mid \bigcap_{j < i} B_j^c\bigg)
	\]
	and the result follows from the lemma.

	Let's prove the lemma by induction on $|S|$. If $S = \varnothing$,
	it trivially follows as
	$\Pr(B_i) \leq x_i \prod_{j \sim i}(1-x_j) < x_i$. Now let's separate the event
	\[\bigcap_{j \in S} B_j^c  = \bigcap_{\substack{j \in S\\j \sim i}}
	B_j^c \cap \bigcap_{\substack{j \in S\\j \not \sim i}} B_j^c  = E \cap F\]
	we think of $E$ as the influence of the "neighbors" in the
	dependence graph and $F$ as the influcence of the non-neighbors.
	We can now write
	\begin{equation}
		\label{eq::lll}
		\Pr(B_i \mid E \cap F) = \frac{\Pr(B_i \cap E \cap F)}{\Pr(E \cap F)} =
		\frac{\Pr(E \mid B_i \cap F)\Pr(B_i \mid F)\Pr(F)}{\Pr(E \mid F) \Pr(F)} \leq
		\frac{x_i\prod_{j \sim i}(1-x_j)}{\Pr(E \mid F)}
	\end{equation}
	If we bound $\Pr(E \mid F) \leq 1$ and use the independence to say
	$\Pr(B_i \mid F)$ is less than what given by the hypothesis.
	Now what is $\Pr(E \mid F)$? Opening it up we get (we abuse the
	notation and say that $j \in E$ if $E$ is taking the intersection with $B_j^c$)
	\[
		\Pr(E \mid F) = \prod_{\substack{j \in E}} \Pr\bigg(B_j^c \mid
		\bigcap_{\substack{k < j\\k \in E}}B_k^c \cap F\bigg) \geq \prod_{j
		\in E} (1-x_j)
	\]
	by the induction hypothesis. Substituting in the inequality
	[\ref{eq::lll}] we prove the lemma.
\end{proof}

\begin{remark}
	We cannot take the graph to be degenerate in the symmetric case as
	we need the I.H. to compute $\Pr(E \mid F)$.
\end{remark}

\begin{remark}
	In the hypothesis, we didn't use full mutual independence, only that
	$\Pr(B_i \mid F) \leq \Pr(B_i)$. Using only this requirement,
	we call it \it{Lopsided LLL}.
\end{remark}

\begin{corollary} [Symmetric LLL]
	Suppose  that in the hypothesis of [\ref{trm:lll}], we had $\max_i
	|N(B_i)| \leq d$. That is the dependence graph $D$
	is such that $\Delta(D) \leq d$. Suppose moreover that for all $i$,
	\[
		\Pr(B_i) \leq \frac{1}{e(d+1)}
	\]
	then $\Pr(\bigcap_{i \leq m} B_i^c) > 0$.
\end{corollary}
\begin{proof}
	Take $x_i = 1/(d+1)$, and note that
	\[
		\Pr(B_i) \leq \frac{1}{e(d+1)} \leq \frac{1}{(d+1)}\bigg(1 -
		\frac{1}{d+1}\bigg)^d \leq x_i \prod_{j \sim i}(1 - x_j).
	\]
\end{proof}

The LLL is a pearl that should always be within our hearts. But there
is a even simpler theorem which we must never forsake.

\begin{theorem} [Dependent Random Choice]
	\label{trm:drc}
	Let $k, s\in \N$ and $G$ be any graph with $n$ vtxs. For any $t \in
	\N$, there exists a set $U \subset V(G)$ with size greater or equal to
	\[
		\frac{(2e(G))^t}{n^{2t - 1}} - \binom{n}{k}\bigg(\frac{s}{n}\bigg)^t
	\]
	such that every set of $k$ vertices of $U$ has $s$ neighbors in common.
\end{theorem}
\begin{proof}
	Choose independently and uniformly $t$ vertices from $V(G)$ with
	possible repetition. Call this random set $T$ and
	let $X = |N(T)|$. Let $Y$ be the number of $k$-sets in $N(T)$ with
	less than $s$ common neighbors. Let's compute $\Ex X$ and $\Ex Y$. We may write
	\[
		X = \sum_{v \in V(G)} \id[v \in N(T)]
	\]
	So $\Ex X = \sum_{v \in V(G)} \Pr(v \in N(T))$. Now if $v \in N(T)$,
	then every vertex in $T$ belongs to $N(v)$, as we chose u.i.i.d we find
	\[
		\sum_{v \in V(G)} \Pr(v \in N(T)) = \sum_{v \in V(G)} \Pr(T \subset
		N(v)) = \sum_{v \in V(G)} \bigg(\frac{d(v)}{n}\bigg)^t \geq n
		\bigg(\frac{2e(G)}{n^2}\bigg)^t
	\]
	where the last inequality follows by Jensen.

	Now what about $Y$? To ease notation, let (only this once)
	$(V(G))_{[k]} = \binom{V(G)}{k}$, then
	\[
		Y = \sum_{\substack{S \in (V(G))_{[k]}\\|N(S)| < s}} \id[S \subset
		N(T)] = \sum_{\substack{S \in (V(G))_{[k]}\\|N(S)| < s}} \id[T \subset N(S)]
	\]
	taking expectation and upperbounding by all $k$-sets, we get
	\[
		E[Y] \leq \binom{n}{k} \bigg(\frac{s}{n}\bigg)^t.
	\]

	We have that
	\begin{equation}
		\label{eq:drc_expec}
		\Ex [X - Y] \geq \frac{(2e(G))^t}{n^{2t - 1}} -
		\binom{n}{k}\bigg(\frac{s}{n}\bigg)^t = a
	\end{equation}

	so there exists an instance of $T$, such that by removing a vertex
	from each bad $k$-set in $N(T)$, we are still
	left with $a$ vertices.
\end{proof}

\subsubsection{Applications}
Let's use the LLL [\ref{trm:lll}] to prove lowerbounds on Ramsey.

Erdös showed that, in his majestic use of the probabilistic method,
the following theorem.

\begin{theorem}
	Consider a random coloring of the edges of $K_N$ in red and blue,
	each chosen independently and uniformly. If $X$ is the number
	of monochromatic cliques of size $k$ and $\Ex X < 1$, then $R(k) >
	N$. In particular, $R(k) \gtrsim 2^{k/2}$.
\end{theorem}
\begin{proof}
	The first statement is a proof in itself, maybe the most beautiful
	proof of all. For the second we only need to do the computation.
	We know
	\[\Ex X = \binom{n}{k}2^{-\binom{k}{2} + 1}  \leq
	\bigg(\frac{en}{k}2^{-k/2}\bigg)^k.\]
	(We lose a bit when doing the binom approximation and we use it). By
	choosing $n$ approprially, this yields
	\[
		R(k) > \frac{k}{e}2^{k/2}.
	\]
\end{proof}

Spencer got a crazy improvement, unbeaten by the ages (50 years
already), using the LLL.
\begin{theorem} (Spencer 77)
	\[
		R(k) > \frac{2k}{e}2^{k/2}.
	\]
\end{theorem}

He also managed to prove Erdös lowerbound on $R(3,k)$ in a much
simpler way - way before Krivilevich's proof [see Theorem
\ref{trm:krivilevich_r3k}].

\subsubsection{Proof of Conlon-Fox-Sudakov}

We will now try to prove Theorem
[\ref{trm:ramsey_bipartite_bounded_degree}] using a twisted
combination of our old friends.
It's a very nice proof.

The first method that comes to mind when trying to find bipartite
graphs is DRC, but it alone won't be strong enough to give us the
upperbound we want. So let's do something a bit better.

\begin{lemma}
	\label{lemma:twisted_drc}
	Let $G$ be a graph on $n$ vtxs and suppose $e(G) \geq c\binom{n}{2}$
	for some $c \leq 1/2$.
	Suppose moreover that $d,k \in \N$ are such that
	\[
		2d \leq k \leq c^{d+2}n.
	\]
	Then, there exists a set $A \subset V(G)$ satisfying
	\begin{enumerate}[label=(\alph*)]
		\item $|A| \geq k$
		\item At most $c^d\binom{|A|}{d}$ $d$-subsets of $A$ have less than
			$k$ neighbors in common.
	\end{enumerate}
\end{lemma}

\begin{proof}
	The proof of this will be exactly the same as the proof of DRC
	[\ref{trm:drc}] but we will take expectation over a different
	expression [\ref{eq:drc_expec}].
	Let $T = (u_1, \dots u_t)$ be a random uniformly independently
	chosen set of vertices. As before we set $X = |N(T)|$,
	and $Y$ the number of bad $d$-sets. Again we have that
	\[
		\Ex X \geq c^tn \quad \text{and} \quad \Ex Y \leq
		\binom{n}{d}\bigg(\frac{k}{n}\bigg)^t
	\]
	Crucially, we want to show that with positive probability $X \geq k$
	and $Y \leq c^d\binom{|X|}{d}$.
	So we will take the mean of a magical r.v with positive expectation.
	Consider $Z$ given by
	\[
		Z = X^t - \frac{(\Ex X)^t}{2} - \frac{(\Ex X)^t}{2}\frac{Y}{\Ex Y}.
	\]
	Note that as $X,Y \geq 0$, we know that $\Ex X^t \geq (\Ex X)^t$, so
	\[
		\Ex Z = \Ex X^t - \frac{\Ex[X]^t}{2} - \frac{\Ex[X]^t}{2} \geq 0.
	\]
	As both $(\Ex X)^t/2$ and $(\Ex X)^tY/(2\Ex Y)$ are positive, it
	follows that with positive probability
	$X^t \geq (\Ex X)^t/2$ and, most importantly,
	\[
		X^t \geq \frac{(\Ex X)^t}{2}\frac{Y}{\Ex Y}.
	\]
	So we found that with probability $> 0$,
	\[
		Y \leq \binom{n}{d}\bigg(\frac{k}{n}\bigg)^t\frac{2X^t}{c^{t^2}n^t}
		\leq \binom{n}{d}\frac{2c^{t(d+2)}X^t}{c^{t^2}n^t}
	\]
	taking $t = d$, we get
	\[
		Y \leq \frac{c^{2d}X^d}{d!} \leq c^d \binom{X}{d}
	\]
	if $d$ is big enough.
	The other bound follows easily,
	\[
		X \geq \frac{\Ex X}{2^{1/d}} \geq \frac{c^dn}{2^{1/d}} \geq k.
	\]
\end{proof}

With this new lemma, let's prove a weaker version of
[\ref{trm:ramsey_bipartite_bounded_degree}] (maybe I can see later
how the stronger one follows.)
\begin{theorem}
	Let $H$ be a $d$-regular bipartite graph on $k$ vtxs. Then
	\[
		r(H) \leq 2^{d+5}k.
	\]
\end{theorem}
\begin{proof}
	Let $n \geq 2^{d+5}k$ and $G$ a graph on $n$ vtxs with $e(G) \geq
	\frac{1}{2}\binom{n}{2}$ - this is a density proof, actually we find
	$H$ in the densest color.
	Suppose $H = A \sqcup B$ and apply lemma [\ref{lemma:twisted_drc}]
	to $G$ with $c = 1/2$. We find a set $A^*$ satisfying the
	conclusions of the lemma.
	We will use the LLL [\ref{trm:lll}] to randomly embed $A$ into $A^*$
	in such a way that for every $b \in B$, $N(b)$ is sent to a good set in $A^*$.
	Because of this, we can trivially find a copy of $H$ in $G$.

	To really use the LLL, we cannot actually embed it ramdomly, this
	would kill all the mutual independence of the events. What we actually do is
	a random assignment.

	Let $f:A \to A^*$ be a random map. We will define two types of bad
	events, collisions and bad sets. For each $u,v \in A$,
	define the events
	\[
		F_{u,v} = \{f(u) = f(v)\}
	\]
	...
	\[
		E_b = \{f(N(b)) \text{ is a bad $d$-set}\}.
	\]
	Amazingly here, if non of the $F_{u,v}$ occur, no neighborhood is
	mapped to less than $d$ elements,
	so we don't even need to care that $f(N(b))$ has the right size. Now
	for the dependence graph, by how we chose our probability
	space, we have that disjoint events (on the vertex of $H$) are non-edges:
	\begin{itemize}
		\item $F_{u,v} \sim F_{u',v'}$ if $\{u,v\} \cap \{u',v'\} \neq \varnothing$.
		\item $F_{u,v} \sim E_b$ if $\{u,v\} \cap N(b) \neq \varnothing$.
		\item $E_b \sim E_{b'}$ if $N(b) \cap N(b') \neq \varnothing$.
	\end{itemize}
	We need to find $x_F$ and $x_E$ satisfying the hypothesis of
	\ref{trm:lll}. We know that
	\[
		\Pr(F_{u,v}) = \frac{1}{|A^*|} \leq \frac{1}{k}
	\]
	and
	\[
		\Pr(E_b) \leq c^d.
	...
	\begin{align*}
		\Pr(E_b) \leq c^d &\leq x_E (1 - x_E)^{d^2}(1 - x_F)^{d|A|}\\
		\Pr(F_{u,v}) \leq \frac{1}{|A^*|} &\leq x_F (1-x_E)^{2d}(1 - x_F)^{2|A|}
	\end{align*}
	so we really should check which is biting us by trying both $x_E$
	close to $\Pr(E_b)$ or $x_F$ close to $\Pr(F_{u,v})$.
	In this case, the collisions are biting. Let $x_F = 2/|A^*|$, then
	(we might have to take $A^*$ bigger here this is why we get a $2^{d
	+ 6}$, we do $|A^*| = 8|A|$)
	\[
		\frac{1}{|A^*|} \leq \frac{2}{|A^*|}(1 - x_E)^{2d}e^{-2{|A|/|A^*|}},
	\]
	So we need
	\[
		(1 - x_E)^{2d} \geq \frac{1}{2e^{1/8}}.
	\]
	From the first inequality we also get
	\[
		c^d \leq x_E(1 - x_E)^{d^2}e^{-d|A|/|A|^*} \leq x_E e^{-x_E d^2}e^{-d/8}
	\]
	so taking $x_E = (ce^{1/8})^d$ we get the result when $d$ tends to infinity.

\end{proof}

\subsection{Erd\H{o}s-Rogers numbers and Open Problems}

Let's look into the very nice generalization of Ramsey numbers, the
Erd\H{o}s-Rogers numbers $f_{H,G}(n)$.
Prepare for the mouthful definition.

\begin{definition}
	Given graphs $H$ and $F$, we let $f_{H,F}(n)$ be the maximum $k$ such
	that every $F$-free graph $G$ on $n$ vertices
	contains a $H$-free set of size $k$. In symbolic terms
	\[
		f_{H,F}(n) = \min_{\substack{v(G) = n\\F \not \subset G}}
		\max_{\substack{U \subset V(G)\\ H \not\subset G[U]}} |U|
	\]
\end{definition}

So we are interested in graphs that are $F$-free but have the highest
density (and spread out too)
of copies of $H$. For this reason, the function was most studied for
$H = K_s$ and $F = K_{s+1}$. And this
case, which we usually denote by $f_{s,s+1}(n)$ was solved
assymptoptically by Rob and Julian!

\begin{theorem}
	\[f_{s,s+1}(n) = \Theta(\sqrt{n \log n})\]
\end{theorem}

The lowerbound was proved using a smart theorem on the clique
chromatic number of a graph. The upperbound
(proved by Rob and Julian) showed up after using a smart
(Hefty-Horn-King-Pfender) construction.

Some numbers are already known. From the recent improvements in
off-diagonal ramsey, we get that
\[
	f_{K_2, K_s}(n) \lesssim n^{1/(s-1) + o(1)}
\]
(this is actually equivalent to the ramsey numbers so this is
basically tight). Although we have this result,
much much less is known about $f_{K_3, K_s}$. Using DRC [\ref{trm:drc}]
Benny Sudakov, managed to show
\begin{theorem}
	\[
		f_{K_3, K_5} = \Omega(n^{5/12}).
	\]
\end{theorem}
Note that, in order to prove something like this. That $f_{K_3,
K_5}(n) \geq k(n)$, we must show that
in every graph $G$ with the property that every $k$ set has a
triangle, we can find a $K_5$ (this is not really obvious but follows
from the definition). Let's quickly prove this theorem.

We start off by saying that if our graph has few triangles, then we
can easily find a big set without triangles and contradict the hypothesis.
The following lemma follows from a simple first moment bound.
\begin{lemma}
	Let $G$ be a graph on $n$ vertex and $dn$ edges. Suppose further that
	$k_3(G) = m$ and $p$ is such that
	\[p^3m \leq pn\]
	then we can find a $K_3$ set of size $pn - p^3m$.
\end{lemma}
As $G$ is $K_5$-free by doing erdos-szekeres, we can bound the number
of $K_3$'s to be at most $ndk$.
Using the previous lemma (and setting $p = 1/\sqrt{dk}$) we see that
we must have $dn > (n/k)^3$. So
now that we have a lowerbound on the number of edges of our graph, we
can apply DRC to check that
if there were $t \in \N$ satisfying

\[
	n\bigg(\frac{d}{n}\bigg)^t - \binom{n}{2}\bigg(\frac{k}{n}\bigg)^t > k
\]
we could find a set of size $k$ such that every pair of elements
contain a shared neighborhood of size $k$ as well.
Becase by assumption every set of size $k$ contains a triangle, we
could easily find our $K_5$. Solving this equation for $t = 1$ or $t
= 2$ and putting $d/n = n/k^3$,
we get that if $k < n^{-5/12}$ or else we could find a $K_5$.

Gowers and O. Janzer showed doing basically the blow up construction
with some smart deletion
that $f_{3,5}(n) = O(n^{6/13})$. Very recently Janzer and Sudakov
showed using the magical Hermitian
Unital and putting blow ups of $K_{t,t,\dots, t}$ (s times) in their
edges that $f_(s, s+2)(n) = O(n^{\frac{2s-3}{4s - 5}})$
I should even read this, seems cool!

Not much more is known! It's the open wild west in here!

\subsection{Induced Ramsey Numbers}

In this section we will prove some results in induced ramsey numbers.
It is motivated by
the amazing breakthrough of Marcelo, Lucas, Rafael e João who showed
(with a very technical proof)
that $R_{\text{ind}}(H) \leq 2^{Cv(H)}$ for some universal $C$ and
every graph $H$.

I won't be able to give even a faint resemblance of their proof, so let's focus 
on easier results that came before. First of all, we should define our object.

Given a finite graph $H$, we call $R_{\text{ind}}(H)$ as the minimum $N$,
such that there exists a graph $G$ on $n$ vertices s.t every $2$-coloring 
of the edges of $G$ contains an induced monochromatic copy of $H$. It is a mouthful!

It is not clear even that this is finite. What is most amazing is that $G(N,1/2)$ works for every $H$
if $N$ is big enough!

\begin{theorem}
    Let $H$ be a graph on $k$ vertices and $N \geq N_0(k) = O(k^2)$ be sufficiently big. Then, with positive probability,
    every $2$-coloring of the edges $G(N,1/2)$ contains an induced monochromatic copy of $H$.
\end{theorem}

The idea here is to use an assymetric container lemma for colorings! More precisely, we want to show 
\[
    \Pr(\exists c : E(G(N,1/2)) \to \{R,B\} \text { without induced monochromatic copy of } H) 
    \leq \sum_{C \in \cC} \Pr(...) \to 0
\]

To deal with induced copies we will define a smart hypergraph that encodes them. Let $\HH$ ve a bipartite hypergraph  with vertex parts $V_0$ and $V_1$ each being a copy of $E(K_n)$. 
We will think of $V_0$ as encoding the non-edges of our graph and $V_1$ encoding the edges. 
Let $E(\HH)$ be the sets that encode an induced copy of $H$. That is $e \in E(\HH)$ if $e \cap V_1$ are the edges of some copy of $H$ and 
$e \cap V_0$ are exactly the non-edges. Note that if $G \subset K_n$ contains no induced 
copy of $H$, then $(G^c, G)$ is an independent set in $\mathcal{H}$.

Associated with this hypergraph, Balogh and Samotij proved the following container theorem 
\begin{theorem}
    For every $H$ and its associated $\HH$, there exists a family of containers $\cC$ satisfying 
    \begin{enumerate}
        \item $|\cC| \leq \exp(O_k(N^2))$
        \item $\forall (G_0, G_1)$ not containing and induced $H$, there exists $(C_0, C_1)$ with 
            $G_0 \subset C_0$ and $G_1 \subset C_1$.
        \item $(C_0, C_1)$ contain $o(n^k)$ induced copies of $H$ for every choice of containers.
    \end{enumerate}
\end{theorem}

Now the idea is as usual, we show that if $C_0$ and $C_1$ are small a bit small, than $G(N,1/2)$ must 
be avoiding a positive proportion of edges which happens with super small probability. On the other 
case that they are big, we will find many induced monochromatic copies, providing a contradiction.

\begin{proof}
    Note that any coloring of the edges of $G(N,1/2)$ in red and blue yields a triple 
    $(G^{c}, G_R, G_B)$ of disjoint edges. If our coloring is free of monochromatic induced 
    $H$, it must be that $(G^c, G_R)$ and $(G^c, G_B)$ are independent sets in $\mathcal{H}$. In particular 
    \[
        \Pr(\exists G_B \cup G_R = G, \text{ no induced MC } H) \leq 
        \sum_{(C_0,C_B,C_R)} \Pr(G^c \subset C_0, G_B \subset C_B \cup G_R \subset C_R).
    \]
    If $|C_0| \leq (1-\delta)\binom{n}{2}$ or $|C_B \cup C_R| \leq (1-\delta) \binom{n}{2}$
    we are very happy, as it would imply $G(N,1/2)$ avoids a positive proportion of edges.

    Let's show that this is always the case. Suppose for sake of contradiction that 
    $|C_0|$ and $|C_B \cup C_R|$ are big. In particular thinking of these containers 
    as sets of $E(K_n)$, we have  that $|C_0 \cap (C_B \cup C_R)| \geq (1-2\delta)\binom{n}{2}$.
    Let $k = v(H)$, by supersaturation we have $\Omega(n^{R(k)})$ copies of $K_{R(k)}$ with 
    these edges. Because they have color red or blue, each of them contain a MC copy 
    of $K_k$, yielding $\Omega(n^{k})$ copies of those. Now, all these MC copies 
    also belong to $C_0$, so we can choose the non-edges in such way that they make up 
    induced copies. This contradicts our choice of containers! After all, we get 

    \[
        \Pr(\exists G_B \cup G_R = G, \text{ no induced MC } H) \leq \exp(o_k(N^2)) 2^{-\delta N^2} \to 0. 
    \]
    finishing the proof!
\end{proof}
