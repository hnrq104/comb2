...
\begin{proof}[Proof of LLL \ref{trm:lll}]
	...
	\end{equation}
	If we bound $\Pr(E \mid F) \leq 1$ and use the independence to say $\Pr(B_i \mid F)$ is less than what given by the hypothesis.
	Now what is $\Pr(E \mid F)$? Opening it up we get (we abuse the notation and say that $j \in E$ if $E$ is taking the intersection with $B_j^c$)
	\[
		\Pr(E \mid F) = \prod_{\substack{j \in E}} \Pr\bigg(B_j^c \mid \bigcap_{\substack{k < j\\k \in E}}B_k^c \cap F\bigg) \geq \prod_{j \in E} (1-x_j)
	\]
	by the induction hypothesis. Substituting in the inequality [\ref{eq::lll}] we prove the claim.
\end{proof}

\begin{remark}
	We cannot take the graph to be degenerate in the symmetric case as we need the I.H. to compute $\Pr(E \mid F)$.
\end{remark}

\begin{corollary} [Symmetric LLL]
	Suppose  that in the hypothesis of [\ref{trm:lll}], we had $\max_i |N(B_i)| \leq d$. That is the dependence graph $D$ 
...
\begin{proof}
	...
	\[
		\Pr(B_i) \leq \frac{1}{e(d+1)} \leq \frac{1}{(d+1)}\bigg(1 - \frac{1}{d+1}\bigg)^d \leq x_i \prod_{j \sim i}(1 - x_j).
	\]
\end{proof}
...
\begin{theorem} [Dependent Random Choice]
	Let $k, s\in \N$ and $G$ be any graph with $n$ vtxs. For any $t \in \N$, there exists a set $U \subset V(G)$ with size greater or equal to
	\[
...
\end{theorem}
\begin{proof}
	Choose independently and uniformly $t$ vertices from $V(G)$ with possible repetition. Call this random set $T$ and
	let $X = |N(T)|$. Let $Y$ be the number of $k$-sets in $N(T)$ with less than $s$ common neighbors. Let's compute $\Ex X$ and $\Ex Y$. We may write
	\[
		X = \sum_{v \in V(G)} \id[v \in N(T)]
	\]
	So $\Ex X = \sum_{v \in V(G)} \Pr(v \in N(T))$. Now if $v \in N(T)$, then every vertex in $T$ belongs to $N(v)$, as we chose u.i.i.d we find
	\[
		\sum_{v \in V(G)} \Pr(v \in N(T)) = \sum_{v \in V(G)} \Pr(T \subset N(v)) = \sum_{v \in V(G)} \bigg(\frac{d(v)}{n}\bigg)^t \geq n \bigg(\frac{2e(G)}{n^2}\bigg)^t
	\]
	where the last inequality follows by Jensen.

	Now what about $Y$? To ease notation, let (only this once) $(V(G))_{[k]} = \binom{V(G)}{k}$, then
	\[
		Y = \sum_{\substack{S \in (V(G))_{[k]}\\|N(S)| < s}} \id[S \subset N(T)] = \sum_{\substack{S \in (V(G))_{[k]}\\|N(S)| < s}} \id[T \subset N(S)]  
	\]
	...
	We have that
	\[
		\Ex [X - Y] \geq \frac{(2e(G))^t}{n^{2t - 1}} - \binom{n}{k}\bigg(\frac{s}{n}\bigg)^t = a
	\]
	so there exists an instance of $T$, such that by removing a vertex from each bad $k$-set in $N(T)$, we are still 
