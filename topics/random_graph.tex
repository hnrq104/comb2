...
\begin{proof}[Proof of \ref{prop:main_prop_chr_concentration}]
	...
	\[
		d(B_e) \leq
		\begin{cases}
			O(d) + k \cdot O(\log n) = O(n^{1/2}\log n)               & \text{
			if } \delta > 1/6                                                  \\
...
\subsection{Thresholds for bounded degree spanning subgraphs}
In this section, we will prove a marvelous theorem by Alon and Furëdi
that gives upperbounds
on the critical probability of spanning subgraphs.

\begin{theorem}[Alon-Furëdi]
	\label{trm:alon_furedi}
	Let $H \subset K_n$ be a graph with $\Delta(H) \leq \Delta$ bounded
	(not depending on $n$),
	if
	\[
        p \gg \bigg(\frac{\log n}{n}\bigg)^{1/\Delta}
	\]
	then
	\[
		\Pr(H \subset G(n,p)) \approx 1.
	\]
\end{theorem}

\begin{remark}
	This theorem is useful for $H$ being hamiltonian cycles or triangle
	factors, or really any spanning subgraph! I've written
	approximately 1 because in truth I wanted to say when $n$ tends to
	infinity, but we can't do that as the graph $H$ is not fixed (we would have
	to consider a sequence of graphs).
\end{remark}

It has a very smart proof. We will divide $H$ into big independent
classes and, using Hall's theorem, iteratively embed
these classes into parts of $G(n,p)$. To embed such a part, we
construct an auxiliary bipartite graph (which will be random with
independent edges) and show that it satisfies Hall's condition with
very high probability.

For our strategy to work, we need a nice little lemma.
\begin{lemma}
	Let $H$ be a $n$-vtx graph with max degree $\Delta = \Delta(H)$.
	Then we can split $V(H)$ into $2\Delta$ independent
	parts $V_1,V_2, \dots V_{2\Delta}$ such that each part is big, i.e
	$|V_i| \geq \floor{n/4\Delta}$ for every $i$.
\end{lemma}

\begin{proof}
	Separate empty sets $V_1, \dots V_{2\Delta}$ and sequentially add
	each vertex $v$ of $H$
	to a part of minimal size which it is allowed, that is $N(v) \cap
	V_i = \varnothing$. Suppose for sake
	of contradiction that in the end, we were left with a part with less
	than $n/4\Delta$ elements.

	Suppose this class is $W$, note that every one of the non-neighbors
	of $W$ could possibly have been
	added to it, but they weren't because they had a even smaller class
	to join. But there are ate least
	$n - \Delta |W| \geq 3n/4$ such elements. Each was added to a class
	of size less than $n/4\Delta$,
	but there are at most $2\Delta$ such classes, so we could have at
	most $n/2$ such elements, absurd!
\end{proof}

Let's give continuity to the proof of the theorem.

\begin{proof}[Proof of \ref{trm:alon_furedi}]
	Apply the previous lemma to $H^2$ to find independent classes $V_1,
	\dots V_{\Delta^2}$
	such that for every $i$, $|V_i| \geq n/4\Delta^2$ and, crucially,
	every element the distance between
	elements $u,w \in V_j$ is at least $3$ (this is important when
	definining the auxiliary graph).

	Now fix a partition $W_1, \dots, W_{\Delta^2}$ of $[n]$ with $|W_i|
	= |V_i|$ and consider $G(n,p)$ over it.
	We will embed each part $V_i$ of $H$ into the corresponding $W_i$
	sequentially. So our objective is to find a bijection $f_i : V_i \to W_i$,
	that respects neighbors of previous choices. That is, given $v \in
	V_i$, suppose it has a neighbor $u \in V_j$ with
	$j < i$, it must be that $\{f_i(v), f_{j}(u)\}$ is an edge in $G(n,
	p)$. To this end, embed $V_1$ into $W_1$ arbitrarily,
	and for each $i > 1$ and $v \in V_i$, define the allowed images of
	$v$ to be the set
	\[
		A(v) = \{w \in W_i : \forall i < j, \forall u \in N(v) \cap V_j,
		\{f_j(u), w\} \in G(n,p)\}.
	\]
	We may consider this an auxiliary bipartite graph $F$ on $(V_i,W_i)$,
	where the neighborhood of each $v \in V_i$ is $A(v)$. Now, notice
	that for $w \in A(v)$,
	$w$ must be connected to the previous at most $\Delta$ neighbors
	chosen. So an edge
	in $F$ occurs with probability at least $p^\Delta$. Note also, that
	two edges of the form
	$\{v_1,w_1\}$ and $\{v_2,w_2\}$ are completely independent if $w_1 \neq w_2$
	(as they depend on disjoint edges of $G(n,p)$). If $w_1 = w_2$, the
	edges keep being independent,
	since there are no paths of length $2$ connecting vertices in $V_i$.
	Because of how we chose $p$,
	with very high probability $F$ satisfies Hall's condition (each edge
	is present with prob at least $\log n/ n$), so we can always find
	such bijection
	and we are done!
\end{proof}

I wasn't really nitty picky on proving that Hall's hold with high
probability here, as it is not
the crux of the proof. But I will do it for completion.

\begin{lemma}
	Let $(A,B)$ be a bipartite graph with $n = |A| = |B|$. Suppose that
	for every set $X \subset A$
	we have that $N(X) \geq |X|$, then there exists a perfect matching in $(A,B)$.
\end{lemma}

\begin{proof}
	We proceed by induction. If $n = 1$, the result follows easily. For bigger $n$,
	call a set $X \subsetneq A$ critical if $|X| = |N(X)|$. If $A$
	contains no critical subsets,
	then pick any edge $(x,y)$, and notice Hall's condition still is
	valid for $(A - x, B - y)$
	and we can apply induction. Suppose $A$ contains a critical $X$ (pick $X$ maximal),
	note that as $N(A) = B$,
	it must be that $B - N(X) \subset N(A - X)$. As Hall's condition is
	valid both for $(X, N(X))$
	and $(A - X, B - N(X))$ (by maximality of $X$), we can apply induction on both parts.
\end{proof}

Now to finish the proof, we just need to prove the following lemma,
\begin{lemma}
	Consider a random bipartite graph $G = (A,B)$ with $|A| = |B| = n$
	where each edge $(a,b)$ appears
	with probability $p = C(\log n/n)$. Then $G$ contains a matching
	with high probability.
\end{lemma}
\begin{proof}
	It suffices to check that $G$ satisfies Hall's condition. Fix a set
	$X$ of size $m$,
	the probability that $|N(X)| \leq |X|$ is bounded by
	\[
		\binom{n}{m} (1-p)^{(n-m)m} \leq
		\begin{cases}
			(\frac{en e^{-p(n-m)}}{m})^{m} & m \leq n/2\\
			(\frac{en e^{-pm}}{n - m})^{n - m} & m \geq n/2\\
		\end{cases}
	\]
	So we always get that this probability is bounded by
	\[
		\bigg(ne^{-pn/2}\bigg)^{\min(m, n-m)} \ll 1
	\]
	and is summable on $m$ if $C$ is a big enough constant.
\end{proof}
