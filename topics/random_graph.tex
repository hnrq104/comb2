...
\begin{proof}[Proof of \ref{prop:main_prop_chr_concentration}]
	...
	Now that the strategy is clear, we will choose this $C_i$ randomly
	(but differently depending on $\delta$). If $\delta \leq 1/5$,
	we will use the LLL [\ref{trm:lll}], if $\delta > 1/5$ we will do a
	clever counting argument. Let's focus on the $\delta \leq 1/5$ case.
	...
	\]
	and by the LLL [\ref{trm:lll}] we get our result. Our proof will stop here, because for $\delta > 1/5$,
	we will actually need the result of Luczak stating that we only need to care about $\delta < 3/8$.
\end{proof}
